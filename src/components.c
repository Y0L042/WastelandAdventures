...
static ecs_query_t *query_init_gridposition = NULL;
static ecs_query_t *query_init_turncomponent = NULL;
...
{
    query_init_gridposition =
        ecs_query(world, {.terms = {{ecs_id(GridPosition)}}});

    query_init_turncomponent =
        ecs_query(world, {.terms = {{ecs_id(TurnComponent)}}});

    query_glyph_draw =
        ecs_query(world, {.terms = {{ecs_id(Glyph)}, {ecs_id(Position)}},
                          .cache_kind = EcsQueryCacheAuto,
                          .flags = EcsQueryMatchEmptyTables});

    query_glyph_ghowst_spawn =
        ecs_query(world, {.terms = {{ecs_id(LeaveGlyphGhost)},
                                    {ecs_id(Glyph)},
                                    {ecs_id(Position)}},
                          .cache_kind = EcsQueryCacheAuto,
                          .flags = EcsQueryMatchEmptyTables});
    ...
    query_glyph_fade =
        ecs_query(world, {.terms = {{ecs_id(Glyph)}, {ecs_id(GlyphFade)}},
                          .cache_kind = EcsQueryCacheAuto,
                          .flags = EcsQueryMatchEmptyTables});

    query_camera_move = ecs_query(world, {.terms = {{ecs_id(CameraComponent)}},
                                          .cache_kind = EcsQueryCacheAuto,
                                          .flags = EcsQueryMatchEmptyTables});

    query_grid_move = ecs_query(
        world, {.terms = {{ecs_id(GridPosition)},
                          {ecs_id(GridVelocity)},
                          {ecs_id(Position)},
                          {ecs_id(TAG_TCEnable)},
    ...
    query_player_input = ecs_query(world, {.terms = {{ecs_id(TurnComponent)},
                                                     {ecs_id(TAG_Player)},
    ...
    query_turncounter_increment =
        ecs_query(world, {.terms = {{ecs_id(TurnCountComponent)},
                                    {ecs_id(TurnComponent)},
                                    {ecs_id(TAG_TurnActive)}},
                          .cache_kind = EcsQueryCacheAuto,
                          .flags = EcsQueryMatchEmptyTables});

    query_draw_health_ui = ecs_query(
        world, {.terms = {{ecs_id(TAG_Player)}, {ecs_id(HealthComponent)}},
                .cache_kind = EcsQueryCacheAuto,
                .flags = EcsQueryMatchEmptyTables});

    query_process_traps = ecs_query(
        world, {.terms = {{ecs_id(TAG_Player)}, {ecs_id(HealthComponent)}},
                .cache_kind = EcsQueryCacheAuto,
                .flags = EcsQueryMatchEmptyTables});

    query_process_triggerareas =
        ecs_query(world, {.terms = {{ecs_id(TriggerArea)},
                                    {ecs_id(GridPosition)},
                                    {ecs_id(TurnComponent)},
                                    {ecs_id(TAG_TurnActive)}},
                          .cache_kind = EcsQueryCacheAuto,
    ...
    query_process_visionareas =
        ecs_query(world, {.terms = {{ecs_id(VisionArea)},
                                    {ecs_id(GridPosition)},
                                    {ecs_id(TurnComponent)},
                                    {ecs_id(TAG_TurnActive)}},
                          .cache_kind = EcsQueryCacheAuto,
    ...
    query_process_hurt = ecs_query(
        world, {.terms = {{ecs_id(HealthComponent)}, {ecs_id(HurtComponent)}},
                .cache_kind = EcsQueryCacheAuto,
                .flags = EcsQueryMatchEmptyTables});

    query_process_death = ecs_query(
        world, {.terms = {{ecs_id(HealthComponent)}, {ecs_id(DeathComponent)}},
                .cache_kind = EcsQueryCacheAuto,
                .flags = EcsQueryMatchEmptyTables});

    query_process_entity_delection =
        ecs_query(world, {
                             .terms = {{ecs_id(EntDeletionQueued)}},
                         });

    query_process_ai = ecs_query(
        world, {.terms = {{ecs_id(AIComponent)}, {ecs_id(TAG_TurnActive)}},
                .cache_kind = EcsQueryCacheAuto,
                .flags = EcsQueryMatchEmptyTables});
}
...
{
    ecs_query_fini(query_init_gridposition);
    ecs_query_fini(query_init_turncomponent);
...
{
    ecs_iter_t it = ecs_query_iter(world, query_init_turncomponent);
...
{
    ecs_iter_t it = ecs_query_iter(world, query_init_gridposition);
    while (ecs_query_next(&it))
    {
        ...
        for (int i = 0; i < it.count; i++)
        {
            ...
            grid->arr_coll_layers[x][y] =
                grid->arr_coll_layers[x][y] | gp[i].coll_layer;
...
/* Forward Declaration */
void ent_glyph_ghost_create(ecs_world_t *world, Glyph *glyph, double time,
...
void handler_glyph_ghost_spawn(ecs_world_t *world)
{
    ...
    while (ecs_query_next(&it))
    {
        ...
        for (int i = 0; i < it.count; i++)
        {
            ent_glyph_ghost_create(it.world, &g[i], gg[i].fade_time, gg[i].x,
                                   gg[i].y);
...
void handler_camera_move(ecs_world_t *world)
{
    ...
    while (ecs_query_next(&it))
    {
        ...
        for (int i = 0; i < it.count; i++)
        {
            if (!ecs_is_alive(it.world, cc[i].target_entity))
            {
                continue;
            }

            const Position *target_pos =
                ecs_get_mut(world, cc[i].target_entity, Position);
            if (target_pos)
            {
                Vector2 current_target = cc[i].camera.target;
                Vector2 desired_target =
                    (Vector2){target_pos->x, target_pos->y};
                cc[i].camera.target =
                    Vector2Lerp(current_target, desired_target, lerp_factor);
    ...
    while (ecs_query_next(&it))
    {
        ...
        for (int i = 0; i < it.count; i++)
        {
            ...
            if (grid)
            {
                int coll_mask = 0;
                int coll_layer = 0;
                gridposition_move(&gp[i], gp[i].x + gv[i].x, gp[i].y + gv[i].y,
                                  0);
                old_x = p[i].x;
                old_y = p[i].y;
...
void handler_player_input(ecs_world_t *world)
{
    ...
    while (ecs_query_next(&it))
    {
        ...
        for (int i = 0; i < it.count; i++)
        {
            ...
            if (is_key_buffered(KEY_KP_7) || is_key_buffered(KEY_KP_8) ||
                is_key_buffered(KEY_KP_9))
            {
                g_y -= 1 * mult; // UP
                moved = 1;
            }
            if (is_key_buffered(KEY_KP_1) || is_key_buffered(KEY_KP_2) ||
                is_key_buffered(KEY_KP_3))
            {
                g_y += 1 * mult; // DOWN
                moved = 1;
            }
            if (is_key_buffered(KEY_KP_1) || is_key_buffered(KEY_KP_4) ||
                is_key_buffered(KEY_KP_7))
            {
                g_x -= 1 * mult; // LEFT
                moved = 1;
            }
            if (is_key_buffered(KEY_KP_3) || is_key_buffered(KEY_KP_6) ||
                is_key_buffered(KEY_KP_9))
            {
                g_x += 1 * mult; // RIGHT
                moved = 1;
            ...
            if (moved == 1)
            {
                ...
                ecs_set(it.world, it.entities[i], GridVelocity,
                        {.x = g_x, .y = g_y});
...
void handler_pathfinding(ecs_world_t *world)
{
    ...
    while (ecs_query_next(&it))
    {
        ...
        {
            Grid *grid = gp[i].grid;
            if (!ecs_is_alive(world, target->target))
            {
                continue;
            }
            const GridPosition *gp_target =
                ecs_get(world, target->target, GridPosition);
            if (gp_target == NULL)
            {
            ...
            find_path(&path, grid, gp[i].x, gp[i].y, gp_target->x, gp_target->y,
                      gp[i].coll_mask);
            if (path.count == 0)
            {
                // ecs_remove(it.world, it.entities[i], PathComponent);
...
void handler_process_triggerareas(ecs_world_t *world)
{
    ...
    while (ecs_query_next(&it))
    {
        ...
        for (int i = 0; i < it.count; i++)
        {
            ...
            int success =
                grid_get_entities_in_area(&gpt[i], &ta[i], &entities) == 0;
            if (success)
            {
            }
...
void handler_process_visionareas(ecs_world_t *world)
{
    ...
    while (ecs_query_next(&it))
    {
        ...
        for (int i = 0; i < it.count; i++)
        {
            ...
            dray_init_values(&entities, ecs_entity_t);
            int success =
                grid_get_entities_in_area(&gpt[i], &va[i], &entities) == 0;
            if (success)
            {
            }
...
void handler_process_hurt(ecs_world_t *world)
{
    ...
    while (ecs_query_next(&it))
    {
        ...
        for (int i = 0; i < it.count; i++)
        {
            ...
            {
                (*healthc[i].callback_onhurt)(it.world, it.entities[i],
                                              hurtc[i].hurt);
            }
        }
    }
...
void handler_process_death(ecs_world_t *world)
{
    ...
    while (ecs_query_next(&it))
    {
        ...
        for (int i = 0; i < it.count; i++)
        {
            ...
            {
                (*healthc[i].callback_ondeath)(it.world, it.entities[i],
                                               healthc[i].health);
            }
