...
void pathfind_astar(DRay *path, Grid *grid, int from_x, int from_y, int to_x,
...
#define ASTARNODE_EMPTY                                                        \
    {                                                                          \
        0, 0, 0, 0, 0, 0, 0, 0, 0, NULL, ASTAR_NONE                            \
    }
...
void _astarnode_init(AStarNode *node, int x, int y, int s_x, int s_y, int t_x,
                     int t_y, int coll_layer, AStarNode *parent, Grid *grid);
void _pathfind_get_neighbors(DRay *neighbors, AStarNode ***nodemap,
                             AStarNode *node, int coll_mask);
                    ...
                    int to_y, int coll_mask)
{
    ...
    for (int _x = 0; _x < grid->width; _x++)
    {
        ...
        {
            _astarnode_init(&nodemap[_x][_y], _x, _y, from_x, from_y, to_x,
                            to_y, grid->arr_coll_layers[_x][_y], NULL, grid);
        }
    }
    ...
    while (open_list->count != 0)
    {
        ...
        if ((current_node->x == current_node->t_x) &&
            (current_node->y == current_node->t_y))
        {
            path_found = 1;
            break;
        }
        ...
        for (int i = 0; i < neighbors.count; i++)
        {
            ...
            if (neighbor->state ==
                ASTAR_NONE) /* If neighbor is not on openlist, calculate values
                               and add */
            {
                neighbor->parent = current_node;
                neighbor->g = current_node->g + 1;
                neighbor->h =
                    Vector2Distance((Vector2){neighbor->t_x, neighbor->t_y},
            ...
            else
            {
                if (neighbor->g <
                    (current_node->g +
                     1)) /* If path from node to neighbor is shorter than
                            neighbor's pref path, update */
                {
                    neighbor->parent = current_node;
                    neighbor->g = current_node->g + 1;
                    neighbor->f = neighbor->g + neighbor->h;
                             ...
                             AStarNode *node, int coll_mask)
{
    ...
    for (int i = -1; i <= 1; i++)
    {
        for (int j = -1; j <= 1; j++)
        {
            ...
            if (neighbor->state == ASTAR_CLOSEDLIST)
            {
                continue;
            }
